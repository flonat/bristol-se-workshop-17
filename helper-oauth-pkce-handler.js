...
async function googleOauthHandler(req, res, sessions) {
    try {
        //get the code from query string
        const code = req.query.code;
        const sessionID = req.query.state;
        const codeVerifier = sessions.get(sessionID).code_verifier


        //get the id and access token with the code
        const tokens = await getGoogleAuthTokens(code, codeVerifier);
...
async function getGoogleAuthTokens(code, codeVerifier) {
