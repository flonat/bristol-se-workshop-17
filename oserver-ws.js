...
const oauthurl = require('./helper-oauth-url');
const oauthHandler = require('./helper-oauth-handler');
...
function run() {
    ...
    const authenticateToken = async (req, res, next) => {
        ...
        if (accessToken) {
            // Token exists — user is already authenticated, proceed
            return next();
        }
        ...
        oauthurl.getGoogleOAuthURL().then((googleAuthUrl) => {
            sessions.set(sid, { 'googleAuthUrl': googleAuthUrl })
    ...
    app.get("/auth/google/callback", async (req, res) => {
        try {
            ...
            let { googleUser, tokens } = await oauthHandler.googleOauthHandler(req, res)
            ...
            if (error.message.includes('invalid_grant')) {
                return res.redirect(path.join(__dirname, '/login.html'))// Redirect the user back to the Google Auth URL// Redirect the user back to the Google Auth URL

            } else {
                return res.status(500).send("Authentication failed.");// Handle other errors appropriately
            }
        }
    });
}
